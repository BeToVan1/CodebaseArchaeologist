...
export default function Home() {
  ...
  const executionFacts = localExecutionClaims(selectedSymbol);
      ...
      <section className="workspace">
        ...
        <aside className="detail" aria-label="Selected code details" aria-live="polite" tabIndex={0}>
          ...
          {selected ? <>
            ...
            {selectedSymbol && <section className="local-execution-section" aria-labelledby="local-execution-heading">
              <div className="section-heading"><h3 id="local-execution-heading">Deterministic execution facts</h3><span>{reportOrigin === "imported" ? "Imported · unverified" : "Static analysis"}</span></div>
              <p>Recorded from the function body, not generated by AI or observed at runtime. These facts remain available if AI is unavailable.</p>
              {reportOrigin === "imported" && <p>These are claims from an imported report; their source and provenance have not been independently verified.</p>}
              {executionFacts.length ? <>
                <ol>{executionFacts.map(claim => <li key={claim.id}>
                  <p>{claim.text}</p><small>Provenance: {claim.provenance}</small>
                </li>)}</ol>
                <a href="#selected-source">Inspect the selected symbol’s source ↓</a>
              </> : <p>No local execution facts were recorded for this symbol. This narrow analysis supports only certain body shapes; absence does not establish safety or exception-free behavior.</p>}
            </section>}
            {explanation && <section className="explanation-section">
              ...
              <div className="claims"><h4>Other claims and evidence</h4>{explanation.claims.filter(claim => !executionFacts.includes(claim)).map((claim, index) => <article className={`claim ${claim.classification}`} key={`${claim.classification}:${index}`}>
            ...
            {selectedSymbol && !canInterpretSelected && <section className="ai-interpretation-section" aria-label="AI interpretation unavailable">
              <div className="section-heading"><h3>AI interpretation</h3><span className="interpretation-label">Unavailable</span></div>
              <p className="ai-intro">{importedReport
                ? "AI generation is disabled for imported JSON reports. This site cannot verify the file’s evidence against an analysis retained by the backend. Importing a report does not restore a previous AI explanation."
                : "AI generation is not available for this selection. It requires supported symbol evidence and an enabled AI service; hosted generation also requires a fresh deep analysis with retained evidence."}</p>
              <p className="ai-intro">You can still inspect the static explanation, available execution facts, and source. Selecting a node does not call the model.</p>
              <details>
                <summary>Show a layout sample (not about this code)</summary>
                <div className="ai-result">
                  <p><strong>Handwritten UI sample · not model output · not an explanation of the selected symbol</strong></p>
                  <article><h4>What it does</h4><p>For a hypothetical function returning value.strip().lower(), the explanation would describe those two operations.</p></article>
                  <article><h4>Execution role</h4><p>The sample function strips whitespace, lowercases the result, and returns it. Its callers are unknown.</p></article>
                  <article><h4>Why it may be structured this way</h4><p>The author’s intent is not established by this hypothetical snippet.</p></article>
                  <div className="ai-uncertainties"><h4>Uncertainties</h4><p>Input types and wider application behavior are unknown. This layout sample has no real citations or confidence scores.</p></div>
                </div>
              </details>
            </section>}
            {canInterpretSelected && <section className="ai-interpretation-section">
              ...
              <p className="ai-intro">Generate an interpretation from commit-bound evidence retained by the analyzer. AI can contradict the evidence even when citations are valid. Compare it with the execution facts and source; confidence is not a measured accuracy score.</p>
