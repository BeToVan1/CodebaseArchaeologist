    ...
    <main className="shell">
      <header className="topbar">
        <div className="brand">
          ...
          <div><div className="product-name">Archaeologist</div><div className="repo-name">cosmicpython/code</div></div>
        </div>
      ...
      <section className="workspace">
        <aside className="rail">
          <div className="eyebrow">Repository</div><h1>File map</h1>
          <p className="rail-copy">Explore the first deterministic output from the analyzer.</p>
          ...
          <nav aria-label="Explorer sections">
            <button className="nav-item active"><span>Files</span><strong>{graph?.nodes.length ?? 0}</strong></button>
            <button className="nav-item" disabled><span>Dependencies</span><strong>—</strong></button>
            <button className="nav-item" disabled><span>Risks</span><strong>—</strong></button>
          </nav>
        ...
        <section className="canvas" aria-label="Repository file graph">
          ...
          <div className="node-grid">
            {visibleNodes.map((node, index) => (
              <button className={`file-node ${selectedId === node.id ? "selected" : ""}`} key={node.id} onClick={() => setSelectedId(node.id)} style={{ "--delay": `${index * 45}ms` } as React.CSSProperties}>
                <div className="file-icon">PY</div><div className="node-text"><strong>{filename(node.path)}</strong><span>{folder(node.path)}</span></div><span className="node-arrow">→</span>
              </button>
            ))}
          </div>
          {!visibleNodes.length && <div className="empty">No files match “{query}”.</div>}
          <div className="canvas-note">Dependency lines will appear when Person A adds edges to the graph.</div>
        </section>

        <aside className="detail" aria-live="polite">
          <div className="detail-top"><span className="pill">FILE</span><button className="more" aria-label="More actions">•••</button></div>
          {selected ? <>
            ...
            <div className="next-step"><span>Next enrichment</span><p>Symbols, imports, and explanations will populate this panel.</p></div>
