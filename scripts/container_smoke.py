...
def main():
    ...
    server = subprocess.Popen(
        ...
        env={**os.environ, "ARCHAEOLOGIST_SERVICE_TOKEN": token},
    ...
    try:
        ...
        assert request("/api/analyze", {"repositoryUrl": "https://github.com/pallets/itsdangerous"})[0] == 401
        assert request("/api/analyze", {"repositoryUrl": "https://example.com/a/b"}, True)[0] == 400
        status, graph = request("/api/analyze", {"repositoryUrl": "https://github.com/pallets/itsdangerous"}, True)
        if status != 200:
