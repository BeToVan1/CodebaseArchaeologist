...
import upgrade_oracle_confidence as release


class ConfidenceUpgradeTests(previous_tests.PatternUpgradeTests):
    def setUp(self):
    ...
    def test_unit_changes_only_image_and_uses_separate_backup(self):
        ...
        self.assertEqual(release.BACKUP.name, 'pre-symbol-confidence-v1.service')
        self.assertNotEqual(release.BACKUP, release.updater.common.BACKUP)

    def test_image_pin_matches_owner_validated_archive(self):
        archive = Path(__file__).resolve().parent.parent / 'artifacts/oracle-c0267f400f9440a0bf208c1a57e8b3a6/deep-service.tar'
        ...
        with tarfile.open(archive) as bundle:
            ...
            images = [item for item in index['manifests'] if item.get('annotations', {}).get('io.containerd.image.name') ==
                'docker.io/library/codebase-archaeologist-deep:oracle-c0267f400f9440a0bf208c1a57e8b3a6']
