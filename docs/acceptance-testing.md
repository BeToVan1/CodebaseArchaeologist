...
- No Oracle image, public website, Sites configuration, AI binding, provider request, or quota was changed. Next gate: validate the container, deploy the evidence-capable Oracle image with explicit approval, then add a same-origin Sites Worker route that forwards only reference/node selection and invokes the already-tested Workers AI adapter.
